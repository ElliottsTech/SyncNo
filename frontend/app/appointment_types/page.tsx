...
export default function AppointmentTypesPage() {
  usePageTitle('Appointment Types — Syncno');
  ...
  const fetchRows = useCallback(() => {
    ...
    const colFilters = Object.entries(columnFilters)
      ...
      .join('');
    fetchJson(`${API}/appointment_types?page=${page}&limit=50${sort}${colFilters}`)
  ...
  const columns = [
    {
      ...
      label: 'Name',
      render: (v, row) => (
        <Link href={`/appointment_types/${row.id}`} className="text-blue-600 hover:underline font-medium">{v}</Link>
      ),
    },
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-6">Appointment Types</h1>
      <DataTable
        columns={columns}
        data={rows}
        serverSide
