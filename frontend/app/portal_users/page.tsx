...
export default function PortalUsersPage() {
  usePageTitle('Portal Users — Syncno');
  ...
  const fetchRows = useCallback(() => {
    ...
    const colFilters = Object.entries(columnFilters)
      ...
      .join('');
    fetchJson(`${API}/portal_users?page=${page}&limit=50${sort}${colFilters}`)
  ...
  const columns = [
    {
      ...
      render: (v, row) => <Link href={`/portal_users/${row.id}`} className="text-blue-600 hover:underline font-medium">{v}</Link>,
    },
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-6">Portal Users</h1>
      <DataTable
        columns={columns}
        data={rows}
        serverSide
